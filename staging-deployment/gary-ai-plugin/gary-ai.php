...
class GaryAI {
    ...
    public function activate() {
        try {
            ...
            error_log('Gary AI: Plugin activation failed: ' . $e->getMessage());
    ...
    public function deactivate() {
        try {
            ...
            // Clear transients
            global $wpdb;
            $wpdb->query("DELETE FROM {$wpdb->options} WHERE option_name LIKE '_transient_gary_ai_%'");
            $wpdb->query("DELETE FROM {$wpdb->options} WHERE option_name LIKE '_transient_timeout_gary_ai_%'");
            
            // Disable chatbot to prevent frontend display
            update_option('gary_ai_chatbot_enabled', 0);
            ...
            error_log('Gary AI: Error during deactivation: ' . $e->getMessage());
        }
    }
}
