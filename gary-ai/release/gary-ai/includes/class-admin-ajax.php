...
class GaryAIAdminAjax {
    ...
    public function handleTestConfiguration() {
        ...
        try {
            ...
            $datastore_id = sanitize_text_field($input['datastore_id'] ?? '');
            
            if (empty($api_key) || empty($agent_id) || empty($datastore_id)) {
                wp_send_json_error('Missing required configuration fields');
                return;
            }
            ...
            $agent_id_property = $reflection->getProperty('agent_id');
            $datastore_id_property = $reflection->getProperty('datastore_id');
            
            $api_key_property->setAccessible(true);
            $agent_id_property->setAccessible(true);
            $datastore_id_property->setAccessible(true);
            
            $api_key_property->setValue($client, $api_key);
            $agent_id_property->setValue($client, $agent_id);
            $datastore_id_property->setValue($client, $datastore_id);
    ...
    public function handleChatMessage() {
        ...
        try {
            ...
            // Send message to AI
            $response = $ai_client->sendMessage($message, [
